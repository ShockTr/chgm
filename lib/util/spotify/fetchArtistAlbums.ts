...
export default async function fetchArtistAlbums(artistId:string, include_groups: groups[], token?:string) : Promise<AlbumObjectSimplified[]>{
    ...
    let current = await fetch(`https://api.spotify.com/v1/artists/${artistId}/albums?limit=50&market=US&include_groups=${include_groups.join(",")}`, {headers: {Authorization: `Bearer ${realToken}`}}).then(r => r.json()) as ArtistsAlbumsResponse
    final = current.items
