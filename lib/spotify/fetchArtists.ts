...
import MultipleArtistsResponse = Spotify.MultipleArtistsResponse;

/**
 * @param ids IDs of the artists to fetch. max50
...
export default async function fetchArtists (ids: string[], token?:string) {
    ...
    let response = await fetch(url, {headers: {Authorization: `Bearer ${realToken}`}}).then(r => r.json()) as MultipleArtistsResponse
